...
*viridis:* for better color scales.
*FedData:* enables us to download geological and environmental data
...
production_value <- read_sheet("https://docs.google.com/spreadsheets/d/1sItBSvmMGL69odL6_2XfMSuG1W7eVU5Y9Ms9GAVE5Rk/edit?usp=sharing", sheet = 8)

cadastre_1800 <- cadastre_1800 %>% # quick cleanup of the data to remove any data without an owner
  filter(!is.na(Main_person))
production_value <- production_value %>% # quick cleanup of the data irrelevant data
  select(!(c("...18","...19")))
```
...
census_1792_total <- census_1792_total %>% 
  filter(quadrant != "Total")

census_1792_quadrants <- census_1792_total %>% 
  left_join(quadrants_cadastre_oxholm_1799) %>% 
  st_as_sf()
```
...
ggplot() +
  ...
  labs(title = "St. Croix population, 1758-1805",
       subtitle = "Based on cadastre ledgers") +
  scale_x_continuous(breaks =  seq(1755, 1855, 5))

ggsave("plots/population_1758-1805.pdf", height= 7, width = 10)
```
...
ggplot(population_total) +
  ...
  scale_x_continuous(name = "Year", breaks = seq(1755,1805,by=5), minor_breaks = seq(1755,1805,by=1))

ggsave("plots/population_1758-1805_improved.pdf", height= 7, width = 10)
```
...
ggsave("plots/population_1758-1805_groups.pdf", height= 7, width = 10)
```
...
quadrant_data %>%
  ...
  geom_area() +
  labs(title = "Rural / Urban population distribution on St. Croix, 1758-1805",
       subtitle = "Based on cadastre ledgers.") +
  scale_y_continuous(labels = scales::percent_format(accuray=1, scale = 1), breaks = seq(0,100,by=10)) +
  scale_x_continuous(name = "Year", breaks = seq(1755,1805,by=5))
...
ggplot(temp) +
  ...
  labs(title = "Net Population Growth, St. Croix, 1758-1805",
       subtitle = "Based on cadastre ledgers") +
  scale_y_continuous(name = "Net Growth", breaks = seq(-2000, 4000, by=500)) +
  scale_x_continuous(name = "Year", breaks = seq(1755,1805,by=5), minor_breaks = seq(1758,1805,by=1))
...
ggplot() +
  ...
  scale_x_continuous(name = "Year", breaks = seq(1755,1805,by=5), minor_breaks = seq(1758,1805,by=1)) +
  scale_color_manual(values=c("#00BA38", "#F8766D", "#000000", "#619CFF"), labels=c("Enslaved", "Free Black", "Total", "White"))
...
ggplot(temp) +
  ...
  labs(
    ...
    y = "Population", 
    color = "Pop. group", 
    title = "Population development by quadrant, 1758-1805") +
  facet_wrap("quadrant") +
  scale_x_continuous(breaks =  seq(1750, 1805, 10))
...
ggplot(temp) +
  ...
  labs(
    ...
    y = "Population", 
    color = "Pop. group", 
    title = "Population by quadrant as a percentage, 1758-1805", 
...
ggplot(temp) +
  ...
  labs(
    ...
    y = "Population", 
    color = "Pop. group", 
    title = "Population of land districts as a percentage, 1758-1805", 
    subtitle = "Growth relative to 1758 for white and enslaved and 1772 for free blacks.") +
  scale_y_continuous(labels = scales::percent_format(accuray=1, scale = 1)) +
...
ggplot(temp) +
  ...
  labs(
    ...
    y = "Population", 
    color = "Pop. group", 
    title = "Population of urban areas as a percentage, 1758-1805", 
    subtitle = "Growth relative to 1758 for white and enslaved and 1772 for free blacks.") +
...
quadrant_data_sf %>% 
  ...
  ggplot()+
  geom_sf(aes(fill = growth_percentage)) +
  geom_sf_text(aes(label = round(growth_percentage, 1)), size = 3.5, color = "black") +
  theme_void() +
...
quadrant_data_sf %>% 
  ...
  facet_wrap(~ year) +
  theme_void() +
  labs(title = "St. Croix population pct. growth, 1792-1805",
       subtitle = "Based on cadastre ledgers",
       fill = "Pct. change in population") +
  theme(plot.title = element_text(vjust=6), # Creating some slight space between the titles from the map.
        plot.subtitle = element_text(vjust=6))+
...
quadrant_data %>%
  ...
  geom_area() +
  geom_hline(yintercept = 50, linetype = "dashed") +
  labs(title = "Gender distribution of the white population on St. Croix, 1772-1805",
       subtitle = "Based on cadastre ledgers.") +
  scale_y_continuous(labels = scales::percent_format(accuray=1, scale = 1)) +
  scale_x_continuous(name = "Year", breaks = seq(1755,1805,by=5))
...
quadrant_data %>%
  ...
  geom_area() +
  geom_hline(yintercept = 50, linetype = "dashed") +
  labs(title = "Gender distribution of the enslaved population on St. Croix, 1793-1805",
       subtitle = "Based on cadastre ledgers.") +
  scale_y_continuous(labels = scales::percent_format(accuray=1, scale = 1)) +
  scale_x_continuous(name = "Year", breaks = seq(1755,1805,by=5))
...
quadrant_data %>%
  ...
  geom_area() +
  geom_hline(yintercept = 50, linetype = "dashed") +
  labs(title = "Gender distribution on St. Croix, 1793-1805",
...
quadrant_data %>%
  mutate(setting = ifelse(quadrant == "Christiansted"|quadrant == "Frederiksted", "Urban", "Rural")) %>%
  group_by(year, setting) %>%
  summarise(total_male = sum(enslaved_male, na.rm = TRUE), total_female = sum(enslaved_female, na.rm = TRUE)) %>% 
  mutate(Male = total_male/(total_male+total_female)*100, Female = total_female/(total_male+total_female)*100) %>%
  gather(Gender, Percentage, Male:Female) %>% 
  ggplot(aes(x=year, y=Percentage, fill=Gender)) + 
  geom_area() +
  geom_hline(yintercept = 50, linetype = "dashed") +
  labs(title = "Gender distribution of the enslaved population on St. Croix, 1793-1805",
       subtitle = "Split by urban and rural areas") +
  scale_y_continuous(labels = scales::percent_format(accuray=1, scale = 1)) +
  scale_x_continuous(name = "Year", breaks = seq(1760,1805,by=2)) +
  facet_wrap(~ setting)
...
Cool now we have a dataframe where we can easily compare the two. However we should also include some of the figures found in the original sources to examine if the data from slavevoyages.org are close to the source material.
```{r}
...
ggplot(slave_voyages) +
  geom_col(aes(x=`Year of arrival at port of disembarkation`,y=`Total disembarked`,
                                     fill=`Flag of vessel`)) +
  labs(title = "Arrival of slave ships to St. Croix , 1758-1805",
       subtitle = "Based on data from slavevoyages.org",
       fill = "Nationality of slave ship") +
  scale_x_continuous(breaks = seq(1760,1805,by=5), minor_breaks = seq(1758,1805,by=1))
...
ggplot(slave_voyages_DWI) +
  geom_col(aes(x=`Year of arrival at port of disembarkation`,y=`Total disembarked`,
                                     fill=`Flag of vessel`)) +
  labs(title = "Arrival of slave ships to St. Croix and unspecifed DWI ports, 1758-1805",
       subtitle = "Based on data from slavevoyages.org",
       fill = "Nationality of slave ship") +
  scale_x_continuous(breaks = seq(1760,1805,by=5), minor_breaks = seq(1758,1805,by=1))
...
ggplot() +
  ...
  geom_col(data = temp, aes(x=year,y=total,
                                     fill=`Flag of vessel`)) +
  labs(title = "Influx of slaves to St. Croix and other unspecifed DWI ports, 1758-1805",
       subtitle = "Based on data from slavevoyages.org",
       fill = "Nationality of slave ship") +
  scale_x_continuous(breaks = seq(1760,1805,by=5), minor_breaks = seq(1758,1805,by=1))
...
ggplot(population_total) +
  ...
  scale_x_continuous(name="Year", breaks = seq(1760,1805,by=5), minor_breaks = seq(1758,1805,by=1)) +
  labs(title = "Influx of enslaves to St. Croix, 1758-1805",
       subtitle = "Based on data from slavevoyages.org and Negerhandelskommissionen",
       fill = "Nationality of slave ship")
...
ggplot(temp) +
  ...
  labs(title = "Can growth be attributed to slave imports alone?",
       subtitle = "Growth calculated from total growth pr. year minus nr. of imported enslaved.") +
...
temp <- slave_voyages_croix %>% #temporary renaming columns to match that of the "population_total" dataframe
  group_by(year) %>%
  summarise(disembarked = sum(disembarked, na.rm = TRUE))
...
# 1792 Census data
```{r}
plantations_1792_union_temp <- plantations_1792 %>% 
  group_by(plantation, owner, quadrant) %>% 
  summarise(geometry = st_union(geometry)) %>%
  ungroup()

st_write(plantations_1792_union_temp, "../QGIS/Data/plantations_1792_joined.shp")

plantations_1792_union <- plantations_1792_union_temp %>% 
  inner_join(census_1792, by = c("plantation", "owner", "quadrant"))
```
...
census_1792 %>%
  ...
  top_n(10) %>% 
    ...
    coord_flip() +
    labs(title = "Largest slave owners on St. Croix in 1792", x = "Slave owner", y = "Nr. enslaved")
...
census_1792 %>%
  ...
  top_n(10) %>% 
    ...
    geom_bar(stat = "identity") +
    coord_flip() +
    labs(title = "Largest slave-owning families on St. Croix in 1792", x = "Slave owner", y = "Nr. enslaved")
...
We should also do the same for estate sizes.
```{r}
census_1792 %>%
  count(owner, wt = acre_total) %>%
  arrange(desc(n)) %>% 
  filter(!is.na(owner)) %>% 
  top_n(10) %>% 
    ggplot(aes(x = reorder(owner, n), y = n)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    labs(title = "Largest landowners on St. Croix in 1792", x = "Name", y = "Acres owned")

ggsave("plots/top_10_estateowners.pdf")

census_1792 %>%
  count(owner_surname, wt = acre_total) %>%
  arrange(desc(n)) %>% 
  filter(!is.na(owner_surname)) %>% 
  top_n(10) %>% 
    ggplot(aes(x = reorder(owner_surname, n), y = n)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    labs(title = "Largest land owning families on St. Croix in 1792", x = "Family", y = "v")

ggsave("plots/top_10_estatefamilies.pdf")
```
...
plantations_1792_union %>% 
  ...
  labs(title = "Distribution of plantation owner nationality", subtitle = "Based on 1792 census", fill = "Familiy")

ggsave("maps/nationality_slavefamilies_1792.pdf", height= 6, width = 10)
```

Lastly, lets take a quick look at how many members are recorded for each familiy.
```{r}
census_1792 %>%
  distinct(owner, .keep_all = T) %>% 
  count(owner_surname, sort = T) %>%
  filter(!is.na(owner_surname)) %>% 
  top_n(5)
...
A final thing we should analyse is how large the average enslaved workforce was for each type of plantation.
```{r}
census_1792 %>%
  mutate(field_workers = rowSums(across(starts_with("field_worker")), na.rm = TRUE),
         non_field_workers = rowSums(across(starts_with("field_worker")))-total) %>%
    filter(!if_all(c(acre_sugar,acre_cotton), is.na)) %>% # filter plantations where all no acres are used for sugar or crops (usually new plantations that haven't existed for long enough)
  mutate_at(vars(acre_sugar, acre_cotton), ~replace_na(., 0)) %>% 
  mutate(plantation_type = ifelse(acre_sugar > 0 & acre_cotton <= 10 , "Sugar", ifelse(acre_cotton > 0 & acre_sugar <= 10, "Cotton", "Mixed"))) %>% 
  group_by(plantation_type) %>% 
  summarise(mean(field_workers), mean(field_workers/acre_total))
```


# Age distribution
## From 1754-1804
Using the cadastre data we can't get a super detailed view of the age of the enslaved population, but we can get a very general one.

In the following we will examine how large the three main population groups of "enslaved capable", "halv-voxne" and "slaver under 12" throughout the period.

```{r}
quadrant_data %>%
  group_by(year) %>%
  summarise(Capable = sum(enslaved_capable, na.rm=TRUE), Teen = sum(enslaved_teen, na.rm=TRUE), Under_12 = sum(enslaved_child, na.rm=TRUE), Unknown = sum(enslaved_manqueron, enslaved_bosal,enslaved_domestic, na.rm=TRUE), total = (Capable+Teen+Under_12+Unknown)) %>% 
  mutate(Capable = Capable/total*100, Teen = Teen/total*100, Under_12 = Under_12/total*100, Unknown = Unknown/total*100) %>%
  gather(Age, Percentage, Capable:Unknown) %>% 
  ggplot(aes(x=year, y=Percentage, fill=Age)) + 
  geom_area() +
  labs(title = "Age distribution on St. Croix, 1758-1805",
       subtitle = "Based on cadastre ledgers. Note: Manqurons, Busals and Domestic slaves\nare all included under 'Unknown'") +
  scale_y_continuous(labels = scales::percent_format(accuray=1, scale = 1)) +
  scale_x_continuous(name = "Year", breaks = seq(1755,1805,by=5)) +
  scale_fill_manual("Age Groups", values = c("Capable" = "#F8766D", "Teen" = "#00BA38", "Under_12" = "#619CFF", "Unknown" = "Grey"))

ggsave("plots/Age_groups_1758_1805.pdf")
```

Does any thing change if we split this data into urban or rural areas?
```{r}
quadrant_data %>%
  mutate(setting = ifelse(quadrant == "Christiansted"|quadrant == "Frederiksted", "Urban", "Rural")) %>%
  group_by(year, setting) %>%
  ...
  ggplot(aes(x=year, y=Percentage, fill=Age)) + 
  geom_area() +
  labs(title = "Age distribution on St. Croix by setting, 1758-1805",
       subtitle = "Based on cadastre ledgers. Note: Manqurons, Busals and Domestic slaves\nare all included under 'Unknown'") +
  scale_y_continuous(labels = scales::percent_format(accuray=1, scale = 1)) +
  scale_x_continuous(name = "Year", breaks = seq(1755,1805,by=10)) +
  scale_fill_manual("Age Groups", values = c("Capable" = "#F8766D", "Teen" = "#00BA38", "Under_12" = "#619CFF", "Unknown" = "Grey")) +
  facet_wrap(~ setting)

ggsave("plots/Age_groups_setting_1758_1805.pdf")
```
...
# New pivot longer - merges "under 5" and "between 5 and 10" age groups. That way every age group is 10 years (except the last "over 60")
age_data_1804_long <- age_data_1804 %>% 
  mutate(age_male_0_to_10 = age_male_under_5+age_male_5_to_10,
         age_female_0_to_10 = age_female_under_5+age_female_5_to_10) %>% 
  select(!c(age_female_5_to_10, age_female_under_5, age_male_under_5, age_male_5_to_10)) %>% 
  pivot_longer(cols = starts_with("age_"),  # Selects columns starting with "age_"
                          names_to = c("gender", "age_group"),  # Separates the column names into gender and age group
                          names_pattern = "age_(male|female)_(.*)",  # Regex pattern to capture gender and age group
                          values_to = "count")

#To make things work later, we need to make our age group a factor
# age_order <- c("under_5", "5_to_10", "10_to_20", "20_to_30", "30_40", "40_50", "50_60", "over_60") # Old original order
age_order <- c("0_to_10", "10_to_20", "20_to_30", "30_40", "40_50", "50_60", "over_60") # New order
...
ggplot(age_data_1804_long, aes(x = age_group, fill = gender, y = ifelse(gender == "male",-count,count))) + 
  ...
  labs(
    ...
    y = "Population", 
    fill = "Gender", 
    title = "Population Pyramid of St. Croix' enslaved population, 1805")
...
ggsave("plots/pop_pyramid_all_1804.pdf")
```

Looking back at the age groups of the cadastre data we it would be interesting if the 1804 data fit sorta in the same groups. We know from earlier that ca. 25 - 30 % of all enslaved were under 16. Is this still true?
```{r}
age_data_1804 %>% 
  mutate(young = age_male_under_5+age_male_5_to_10+age_male_10_to_20+age_female_under_5+age_female_5_to_10+age_female_10_to_20,
         capable = age_male_20_to_30+age_male_30_40+age_male_40_50+age_male_50_60+age_male_over_60+age_female_20_to_30+age_female_30_40+age_female_40_50+age_female_50_60+age_female_over_60) %>% 
  summarise(Capable = sum(capable, na.rm=TRUE), Young = sum(young, na.rm=TRUE), total = (Capable+Young)) %>% 
  mutate(Capable = Capable/total*100, Young = Young/total*100) %>%
  gather(Age, Percentage, Capable:Young) %>% 
  ggplot(aes(x=Age, y=Percentage, fill=Age)) + 
  geom_col() +
  labs(title = "Age distribution on St. Croix, 1805",
       subtitle = "Based on 1805 census. Capable are defined as being over 20.") +
  scale_y_continuous(labels = scales::percent_format(accuray=1, scale = 1),breaks = seq(0,100,by=10)) +
  scale_fill_manual("Age Groups", values = c("Capable" = "#F8766D", "Young" = "#00BA38"))

ggsave("plots/age_capable_young_1805.pdf")
```

The last part is to examine what the average age on the island was. As we don't know the exact age, we need to use the averaged ages instead. For "over 60" we will simply use "60" as the age.

```{r}
age_mapping <- data.frame(
  age_group = c("0_to_10", "10_to_20", "20_to_30", "30_40", "40_50", "50_60", "over_60"),
  average_age = c(5, 15, 25, 35, 45, 55, 65)
)

# Join the age mapping with the original data
age_data_1804_average <- age_data_1804_long %>%
  left_join(age_mapping, by = "age_group")

# Calculate the weighted average age for each gender
age_data_1804_average <- age_data_1804_average %>%
  group_by(gender) %>%
  summarise(
    total_count = sum(count, na.rm = T),
    weighted_age_sum = sum(average_age * count, na.rm = T),
    average_age = weighted_age_sum / total_count
  )
```
...
# Spatial distribution of objects, population and agriculture
...
## Enslaved pop and field types.
The 1804 and 1792 census also has some interesting figures pertaining to acres allocated to the enslaved population. I wonder if there the whole island was more or less the same, or if there was a difference by area. Let us look.

```{r}
temp <- census_1804_quadrants %>%
  mutate(acres_pr_enslaved = field_for_enslaved/total) %>% 
  select(year, quadrant, acres_pr_enslaved)

temp2 <- census_1792_quadrants %>%
  mutate(acres_pr_enslaved = acre_for_enslaved/total) %>% 
  select(year, quadrant, acres_pr_enslaved) 
...
temp <- temp %>% 
  mutate(year = ifelse(year == 1804, 1805, year))

temp %>%
  ggplot() +
  geom_sf(aes(fill = acres_pr_enslaved)) +
    geom_sf_text(data = temp, aes(label = round(acres_pr_enslaved, 3)), size = 2.5, color = "white") +
  facet_wrap(~ year, ncol = 1) +
  labs(title = "Acres allocated to enslaved by quadrant, 1792 vs. 1805.", fill = "Acres pr. enslaved")

ggsave("maps/acre_enslaved.pdf", height= 6, width = 10)
```

Wow that is a big shift for the worse. It seems the increased population from the end of the slave trade meant that alot fewer acres were available pr. acre. Looking closer at the original figures, there exists a small disparity between the total acres in 1792 and 1804, but the difference is small. The development above can therefor not just be explained by a different way of measuring the total acres available.

As was examined earlier, Oost Ende B and Compagniets Quadrant bot only experienced a marginal increase population during this time. This could ellude to a general trend towards more intensified agriculture utilizing more and more land for their primary crop. Both years include figures pertaining to acres used for sugar. Lets see if there is a change.

```{r}
temp <- census_1804_quadrants %>%
  mutate(acres_sugar = (field_sugar_cultivation+field_sugar_uncultivated)/field_total) %>% 
  select(year, quadrant, acres_sugar)

temp2 <- census_1792_quadrants %>%
  mutate(acres_sugar = acre_sugar/acre_total) %>% 
  select(year, quadrant, acres_sugar)
...
temp %>%
  ggplot() +
  geom_sf(aes(fill = acres_sugar)) +
  geom_sf_text(data = temp, aes(label = scales::percent(round(acres_sugar, 3))), size = 2.5, color = "white") +
  facet_wrap(~ year, ncol = 1) +
  labs(title = "Acres allocated to sugar cultivation by quadrant, 1792 vs. 1805.",  fill = "As Percentage\nof total area") +
  scale_fill_continuous(labels=scales::percent)

```

It does not seems as if there is a huge increase of sugar cultuvation during this time. Is it because less area is allocated for the enslaved?
```{r}
temp <- census_1804_quadrants %>%
  mutate(acres_enslaved = field_for_enslaved/field_total) %>% 
  select(year, quadrant, acres_enslaved)

temp2 <- census_1792_quadrants %>%
  mutate(acres_enslaved = acre_for_enslaved/acre_total) %>% 
  select(year, quadrant, acres_enslaved)
...
temp %>%
  ggplot() +
  geom_sf(aes(fill = acres_enslaved)) +
  geom_sf_text(data = temp, aes(label = scales::percent(round(acres_enslaved, 3))), size = 2.5, color = "white") +
  facet_wrap(~ year, ncol = 1) +
  labs(title = "Acres allocated to enslaved by quadrant, 1792 vs. 1804.",  fill = "As Percentage\nof total area") +
    scale_fill_continuous(labels=scales::percent)

ggsave("maps/acre_enslaved_procent.pdf", height= 6, width = 10)
```

Oh this seems to be the reason! But what did they shift their production too if not sugar production? Perhaps more provisions or more uncultivated / land in fallow. Let us check.
```{r}
# Provision acres
temp <- census_1804_quadrants %>%
  mutate(acres_provision = field_sugar_provision/field_total) %>% 
  select(year, quadrant, acres_provision)

temp2 <- census_1792_quadrants %>%
  mutate(acres_provision = acre_provision/acre_total) %>% 
  select(year, quadrant, acres_provision)
...
temp %>%
  ggplot() +
  geom_sf(aes(fill = acres_provision)) +
  geom_sf_text(data = temp, aes(label = scales::percent(round(acres_provision, 3))), size = 2.5, color = "white") +
  facet_wrap(~ year, ncol = 1) +
  labs(title = "Acres allocated to provisions by quadrant, 1792 vs. 1804.",  fill = "As Percentage\nof total area") +
    scale_fill_continuous(labels=scales::percent)

# Uncultivated / fallow and woodland land
temp <- census_1804_quadrants %>%
  mutate(acres_uncultivated = field_uncultivated/field_total) %>% 
  select(year, quadrant, acres_uncultivated)

temp2 <- census_1792_quadrants %>%
  mutate(acres_uncultivated = (acre_fallow+acre_woodland)/acre_total) %>% 
  select(year, quadrant, acres_uncultivated)
...
temp %>%
  ggplot() +
  geom_sf(aes(fill = acres_uncultivated)) +
  geom_sf_text(data = temp, aes(label = scales::percent(round(acres_uncultivated, 3))), size = 2.5, color = "white") +
  facet_wrap(~ year, ncol = 1) +
  labs(title = "Uncultivated acres by quadrant, 1792 vs. 1804.",  fill = "As Percentage\nof total area") +
    scale_fill_continuous(labels=scales::percent)
```

Hmmm interesting. It would seem more land is left uncultivated. It is hard to say if this is by choice or due to other circumstances - such as the soil having been drained of nutrients or it purposefully left uncultivated to increase nutrients for next year. However, as the 1804 has data on sugar fields left uncultivated, the second possibility does not seem too certain

The 1804 census also has data on housing for the enslaved. How does this look?
```{r}
temp <- census_1804_quadrants %>%
  mutate(enslaved_pr_house = total/houses_enslaved)

ggplot(temp) +
  geom_sf(aes(fill = enslaved_pr_house)) +
  geom_sf_text(data = temp, aes(label = round(enslaved_pr_house, 2)), size = 2.5, color = "white") +
  labs(title = "Enslaved housing by quadrant, 1805.", fill = "Enslaved pr. house")

ggsave("maps/enslaved_housing_1805.pdf", height= 6, width = 10)
```

Let us create a quick histogram to see how this number is calculated (i.e if there are many outliers)
```{r}
census_1804 %>%
  mutate(enslaved_pr_house = total/houses_enslaved) %>% 
  ggplot(aes(x = enslaved_pr_house)) +
  geom_histogram(binwidth = 1) +
  scale_x_continuous(breaks =  seq(0,22,1))

ggsave("plots/enslaved_pr_house_hist.pdf", height= 5, width = 8)
```

We should also examine what areas were more dominated by african vs. creol slaves (ie. slaves imported to the island vs. born on the island). Again the two cenus' can help us.
```{r}
temp <- census_1804_quadrants %>%
  mutate(Creole = native/total,
         African = african/total) %>%
  select(year, quadrant, Creole, African)

temp2 <- census_1792_quadrants %>%
  mutate(Creole = creole/total,
         African = african/total) %>%
  select(year, quadrant, Creole, African)

temp <- temp %>%
  rbind(temp2) %>% 
  pivot_longer(cols = c("Creole", "African"),  # Selects columns starting with "age_"
                          values_to = "pct")
...
temp %>%
  ...
  geom_sf_text(data = temp, aes(label = scales::percent(round(pct, 3))), size = 2.5, color = "white") +
  facet_grid(year ~ name) +
  labs(title = "Creole and african enslaved population, 1792 vs. 1805.",  fill = "As Percentage\nof total enslaved\npopulation") +
    scale_fill_continuous(labels=scales::percent) +
   theme(axis.title=element_blank(),
        axis.text=element_blank(),
        axis.ticks=element_blank())

ggsave("maps/african_creole_quadrant_census.pdf", height= 6, width = 10)
```
...
census_1792 %>%
  mutate_at(vars(acre_sugar, acre_cotton), ~replace_na(., 0)) %>% 
  mutate(plantation_type = ifelse(acre_sugar > 0 & acre_cotton <= 10 , "Sugar", ifelse(acre_cotton > 0 & acre_sugar <= 10, "Cotton", "Mixed"))) %>% 
  mutate(creole = replace_na(creole, 0),
         african = replace_na(african, 0)) %>% 
  mutate(Creole = creole/total,
         African = african/total) %>% 
  group_by(plantation_type) %>% 
  summarise(Creole = mean(Creole),
            African = mean(African))
```

The 1792 census has numbers on how many maroons each plantation had. Lets plot it by plantation.
```{r}
plantations_1792_union %>% 
  filter(!is.na(maroons)) %>% 
  ggplot() +
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=maroons))+
  labs(title = "Number of maroons by plantation", subtitle = "Based on 1792 census", fill = "Count")

ggsave("maps/maroons_1792.pdf", height= 6, width = 10)
```
How does this look as a histogram?
```{r}
temp <- census_1792 %>% 
  mutate(maroons = replace_na(maroons, 0)) %>% # setting NA's to 0
  ggplot(aes(x = maroons)) +
  geom_histogram(binwidth = 1) +
  scale_x_continuous(breaks =  seq(0,20,1)) +
  ylim(c(0, 160)) +
  labs(title = "Number of plantations with maroons", subtitle = "Based on 1792 census", x = "Number of maroons", y = "Number of plantations")

# This part is needed to fix label positions.
binwidth = layer_data(temp) %>% mutate(w=xmax-xmin) %>% pull(w) %>% median

temp +   stat_bin(aes(y=after_stat(count), label=ifelse(after_stat(count)==0,"",after_stat(count))), geom="text", vjust=-1, binwidth=binwidth)

ggsave("plots/maroon_hist.pdf", height= 5, width = 8)
```

## Cotton and sugar distribution
```{r Cotton_production}
#By plantation
plantations_1792_union %>%
  ggplot()+
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=avg_harvest_cotton))+
  labs(title = "Cotton production by plantation, 1792", subtitle = "Based on 1792 census", fill = "Pounds of cotton*") # What is the actual unit?

# By quadrant
census_1792_quadrants %>% 
  ggplot() +
  geom_sf(aes(fill = avg_harvest_cotton)) +
  labs(title = "Cotton production by quadrant, 1792", subtitle = "Based on 1792 census", fill = "Pounds of cotton*") # What is the actual unit?
```
...
plantations_1792_union %>%
  ggplot()+
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=avg_harvest_sugar))+
  labs(title = "Sugar production by plantation, 1792", subtitle = "Based on 1792 census", fill = "Pounds of sugar") # What is the actual unit?

# By quadrant
census_1792_quadrants %>% 
  ggplot() +
  geom_sf(aes(fill = avg_harvest_sugar)) +
  labs(title = "Sugar production by quadrant, 1792", subtitle = "Based on 1792 census", fill = "Pounds of sugar") # What is the actual unit?
```
...
# Productivity and harvest values
To examine productivity we can use the census from 1792 that has data on the average harvest from each plantation.
...
## Yearly crucian production and value
```{r}
colors <- c("Production (pd.)" = "#F8766D", "Auction value (Rdl.)" = "#619CFF")

production_value %>% 
  filter(year >= 1755) %>% 
  ggplot(aes(x = year)) +
  geom_point(aes(y = sugar_pound, color = "Production (pd.)")) +
  geom_line(aes(y = sugar_pound, color = "Production (pd.)")) +
  geom_line(data = filter(production_value, year >= 1755,!is.na(sugar_pound)), aes(y = sugar_pound, color = "Production (pd.)"), linetype="dashed") +
  geom_point(data = filter(production_value, !is.na(sugar_value)), aes(y = sugar_value*10, color = "Auction value (Rdl.)")) +
  geom_line(data = filter(production_value, year >= 1755), aes(y = sugar_value*10, color = "Auction value (Rdl.)")) +
  geom_line(data = filter(production_value, !is.na(sugar_value)), aes(y = sugar_value*10, color = "Auction value (Rdl.)"), linetype="dashed") +
  scale_y_continuous(labels=scales::label_number(big.mark = ".", decimal.mark = ","), name = "Sugar Harvest (pd.)",
    sec.axis = sec_axis(~./10, name="Sugar Value (Rdl.)", labels = scales::label_number(big.mark = ".", decimal.mark = ","))) +
  scale_x_continuous(breaks = seq(1755,1805,by=5), minor_breaks = seq(1755,1805,by=1)) +
  labs(title = "Production and value of St. Croix's sugar production.",
       subtitle = "Prices based on auction prices of brown sugar (in Cph.)",
       color = "Sugar harvest") +
  theme(axis.title.y.left = element_text(margin = margin(r = 10)),
        axis.title.y.right = element_text(margin = margin(l = 10))) +
  scale_color_manual(values = colors)
  
ggsave("plots/Sugar_production_1754_1804.pdf", height= 6, width = 10)
```

To a certain degree this graph sorta looks like the graph overview of the population growth of St. Croix. The two are probably linked to some degree.
...
*** TJEK OVENSTÅENDE IGENNEM ***

To calculate plantation productivity we can simply divide the average harvest with the total planted area or the total amount of enslaved. These numbers we can then weigh by the value of the final product to create a aggregated productivt score by acre and by enslaved. These aggreagted productivty scores can then normalised and then examined in a matrix.

### Unweighted scores
First things first: lets create a clean dataframe with only the relevant columns.
```{r}
productivity <- census_1792 %>% 
  mutate(field_workers = rowSums(across(starts_with("field_worker")), na.rm = TRUE),
         non_field_workers = rowSums(across(starts_with("field_worker")))-total) %>% 
  select(plantation, quadrant, owner, owner_surname, owner_nationality, field_workers, non_field_workers, total, starts_with("avg_harvest"), acre_sugar, acre_cotton, acre_total) %>%
  filter(!if_all(c(avg_harvest_sugar,avg_harvest_rum,avg_harvest_cotton), is.na)) # filter plantations where all avg_harvests are NA (usually new plantations that haven't existed for long enough)
```

Now we can calculate the productivity scores.

We will also convert barrels of sugar to pounds. In the earlier years of the colony the barrels were smaller and in time (as the island be came deforestated) the barrel sizes started becoming larger. There is no standard barrel size, however, the usual numbers thrown around during this time are between 1000-1200 pd. pr. barrel. Let us set it to 1000 to set a conservative estimate.

```{r}
productivity <- productivity %>%
  mutate(worker_productivity_sugar = (avg_harvest_sugar*1000) / field_workers, # 1000 pd = 1 barrel (conservative est.)
         worker_productivity_rum = avg_harvest_rum / field_workers,
         worker_productivity_cotton = avg_harvest_cotton / field_workers,
         acre_productivity_sugar = (avg_harvest_sugar*1000) / acre_sugar, # 1000 pd = 1 barrel (conservative est.)
         acre_productivity_rum = avg_harvest_rum / acre_sugar,
         acre_productivity_cotton = avg_harvest_cotton / acre_cotton)
```

```{r}
# A quick calculation of mean productivity scores
paste("Mean crop-productivity scores:")
paste("Pr. worker - Sugar:", mean(productivity$worker_productivity_sugar, na.rm = T), "Pd. pr. worker", sep=" ")
paste("Pr. worker - Rum:", mean(productivity$worker_productivity_rum, na.rm = T), "XXX pr. worker", sep=" ")
paste("Pr. worker - Cotton:", mean(productivity$worker_productivity_cotton, na.rm = T), "Pd. pr. worker", sep=" ")
paste("Pr. acre - Sugar:", mean(productivity$acre_productivity_sugar, na.rm = T), "Pd. pr. acre", sep=" ")
paste("Pr. acre - Rum:",mean(productivity$acre_productivity_rum, na.rm = T), "XXX pr. acre", sep=" ")
paste("Pr. acre - Cotton:", mean(productivity$acre_productivity_cotton, na.rm = T), "Pd. pr. acre", sep=" ")

paste("")

#And median scores
paste("Median crop-productivity scores:")
paste("Pr. worker - Sugar:", median(productivity$worker_productivity_sugar, na.rm = T), "Pd. pr. worker", sep=" ")
paste("Pr. worker - Rum:", median(productivity$worker_productivity_rum, na.rm = T), "XXX pr. worker", sep=" ")
paste("Pr. worker - Cotton:", median(productivity$worker_productivity_cotton, na.rm = T), "Pd. pr. worker", sep=" ")
paste("Pr. acre - Sugar:", median(productivity$acre_productivity_sugar, na.rm = T), "Pd. pr. acre", sep=" ")
paste("Pr. acre - Rum:", median(productivity$acre_productivity_rum, na.rm = T), "XXX pr. acre", sep=" ")
paste("Pr. acre - Cotton:", median(productivity$acre_productivity_cotton, na.rm = T), "Pd. pr. acre", sep=" ")
```

Now we can create our first graphs to view scores.
```{r}
# Pr. worker productivity scores - histograms
productivity %>% 
  ggplot() +
  geom_histogram(aes(x=worker_productivity_sugar, fill=quadrant), binwidth = 250) +
  labs(title="Crop-productivity of sugar pr. worker - Histogram", fill="Quadrant", x="Pds. of sugar pr. worker")

productivity %>% 
  ggplot() +
  geom_histogram(aes(x=worker_productivity_rum, fill=quadrant)) +
  labs(title="Crop-productivity of rum pr. worker - Histogram", fill="Quadrant", x="XXX of rum pr. worker")

productivity %>% 
  ggplot() +
  geom_histogram(aes(x=worker_productivity_cotton, fill=quadrant), binwidth = 50) +
  labs(title="Crop-productivity of cotton pr. worker - Histogram", fill="Quadrant", x="Pds. of cotton pr. worker")
```

```{r}
# Pr. acre productivy scores - histograms
productivity %>% 
  ggplot() +
  geom_histogram(aes(x=acre_productivity_sugar, fill=quadrant), binwidth = 250) +
  labs(title="Crop-productivity of sugar pr. acre - Histogram", fill="Quadrant", x="Pds. of sugar pr. acre")

productivity %>% 
  ggplot() +
  geom_histogram(aes(x=acre_productivity_rum, fill=quadrant)) +
  labs(title="Crop-productivity of rum pr. acre - Histogram", fill="Quadrant", x="XXX of rum pr. acre")

productivity %>% 
  ggplot() +
  geom_histogram(aes(x=acre_productivity_cotton, fill=quadrant), binwidth = 50) +
  labs(title="Crop-productivity of cotton pr. acre - Histogram", fill="Quadrant", x="Pds. of cotton pr. acre")
```

Finally we can normalize the scores and measure the average productivity scores in a matrix. To do this we need the mean and standard deviation scores.
```{r}
mean_sugar_per_acre <- mean(productivity$acre_productivity_sugar, na.rm = T)
sd_sugar_per_acre <- sd(productivity$acre_productivity_sugar, na.rm = T)

mean_rum_per_acre <- mean(productivity$acre_productivity_rum, na.rm = T)
sd_rum_per_acre <- sd(productivity$acre_productivity_rum, na.rm = T)

mean_cotton_per_acre <- mean(productivity$acre_productivity_cotton, na.rm = T)
sd_cotton_per_acre <- sd(productivity$acre_productivity_cotton, na.rm = T)

mean_sugar_per_worker <- mean(productivity$worker_productivity_sugar, na.rm = T)
sd_sugar_per_worker <- sd(productivity$worker_productivity_sugar, na.rm = T)

mean_rum_per_worker <- mean(productivity$worker_productivity_rum, na.rm = T)
sd_rum_per_worker <- sd(productivity$worker_productivity_rum, na.rm = T)

mean_cotton_per_worker <- mean(productivity$worker_productivity_cotton, na.rm = T)
sd_cotton_per_worker <- sd(productivity$worker_productivity_cotton, na.rm = T)
```

This we can use to calculate the z-score
```{r}
z_score <- function(x, mean, sd) {
  (x - mean) / sd
}

productivity$Z_sugar_per_Acre <- z_score(productivity$acre_productivity_sugar, mean_sugar_per_acre, sd_sugar_per_acre)
productivity$Z_rum_per_Acre <- z_score(productivity$acre_productivity_rum, mean_rum_per_acre, sd_rum_per_acre)
productivity$Z_cotton_per_Acre <- z_score(productivity$acre_productivity_cotton, mean_cotton_per_acre, sd_cotton_per_acre)

productivity$Z_sugar_per_worker <- z_score(productivity$worker_productivity_sugar, mean_sugar_per_worker, sd_sugar_per_worker)
productivity$Z_rum_per_worker <- z_score(productivity$worker_productivity_rum, mean_rum_per_worker, sd_rum_per_worker)
productivity$Z_cotton_per_worker <- z_score(productivity$worker_productivity_cotton, mean_cotton_per_worker, sd_cotton_per_worker)
```

Finally we can graph our results
```{r}
productivity %>%
  ggplot() +
  geom_point(aes(x=Z_sugar_per_worker, y=Z_sugar_per_Acre, color=quadrant)) +
  geom_hline(aes(yintercept=0)) +
  geom_vline(aes(xintercept=0)) +
  labs(title = "Sugar productivity compared to average", x = "Crop-productivity pr. worker", y = "Crop-productivity pr. acre", color = "Quadrant")

ggsave("plots/Sugar_crop-zscores.pdf", height= 6, width = 10)

productivity %>% 
  ggplot() +
  geom_point(aes(x=Z_rum_per_worker, y=Z_rum_per_Acre, color=quadrant)) +
  geom_hline(aes(yintercept=0)) +
  geom_vline(aes(xintercept=0)) +
    labs(title = "Rum productivity compared to average", x = "Crop-productivity pr. worker", y = "Crop-productivity pr. acre", color = "Quadrant")

ggsave("plots/rum_crop-zscores.pdf", height= 6, width = 10)

productivity %>% 
  ggplot() +
  geom_point(aes(x=Z_cotton_per_worker, y=Z_cotton_per_Acre, color=quadrant)) +
  geom_hline(aes(yintercept=0)) +
  geom_vline(aes(xintercept=0)) +
    labs(title = "Cotton productivity compared to average", x = "Crop-productivity pr. worker", y = "Crop-productivity pr. acre", color = "Quadrant")

ggsave("plots/cotton_crop-zscores.pdf", height= 6, width = 10)

```
Interesting. What about the mean z-score by quadrant
```{r}
productivity %>%
  group_by(quadrant) %>%
  summarize(
    mean_worker = mean(Z_sugar_per_worker, na.rm = TRUE),
    mean_acre = mean(Z_sugar_per_Acre, na.rm = TRUE),
    median_worker = median(Z_sugar_per_worker, na.rm = TRUE),
    median_acre = median(Z_sugar_per_Acre, na.rm = TRUE)) %>%
  pivot_longer(cols = c("mean_worker", "mean_acre", "median_worker", "median_acre"),  # Selects columns starting with "age_"
                          names_to = c("mean_median", "worker_acre"),  # Separates the column names into gender and age group
                          names_pattern = "(mean|median)_(worker|acre)",  # Regex pattern to capture gender and age group
                          values_to = "count") %>% 
  pivot_wider(names_from = worker_acre, values_from = count) %>% 
  ggplot() +
  geom_point(aes(x=worker, y=acre, color=quadrant, shape=mean_median)) +
  geom_hline(aes(yintercept=0)) +
  geom_vline(aes(xintercept=0)) +
  labs(title = "Mean and median sugar crop-productivity compared to the average", subtitle = "By quadrant", x = "Crop-productivity pr. worker", y = "Crop-productivity pr. acre", color = "Quadrant")

ggsave("plots/Mean_Sugar_crop-zscores.pdf", height= 6, width = 10)

productivity %>%
  group_by(quadrant) %>%
  summarize(
    mean_worker = mean(Z_rum_per_worker, na.rm = TRUE),
    mean_acre = mean(Z_rum_per_Acre, na.rm = TRUE),
    median_worker = median(Z_rum_per_worker, na.rm = TRUE),
    median_acre = median(Z_rum_per_Acre, na.rm = TRUE)) %>%
  pivot_longer(cols = c("mean_worker", "mean_acre", "median_worker", "median_acre"),  # Selects columns starting with "age_"
                          names_to = c("mean_median", "worker_acre"),  # Separates the column names into gender and age group
                          names_pattern = "(mean|median)_(worker|acre)",  # Regex pattern to capture gender and age group
                          values_to = "count") %>% 
  ...
  geom_hline(aes(yintercept=0)) +
  geom_vline(aes(xintercept=0)) +
    labs(title = "Mean and median rum crop-productivity compared to the average", subtitle = "By quadrant", x = "Crop-productivity pr. worker", y = "Crop-productivity pr. acre", color = "Quadrant")

ggsave("plots/Mean_rum_crop-zscores.pdf", height= 6, width = 10)

productivity %>%
  group_by(quadrant) %>%
  summarize(
    mean_worker = mean(Z_cotton_per_worker, na.rm = TRUE),
    mean_acre = mean(Z_cotton_per_Acre, na.rm = TRUE),
    median_worker = median(Z_cotton_per_worker, na.rm = TRUE),
    median_acre = median(Z_cotton_per_Acre, na.rm = TRUE)) %>%
  pivot_longer(cols = c("mean_worker", "mean_acre", "median_worker", "median_acre"),  # Selects columns starting with "age_"
                          names_to = c("mean_median", "worker_acre"),  # Separates the column names into gender and age group
                          names_pattern = "(mean|median)_(worker|acre)",  # Regex pattern to capture gender and age group
                          values_to = "count") %>% 
  ...
  geom_hline(aes(yintercept=0)) +
  geom_vline(aes(xintercept=0)) +
      labs(title = "Mean and median cotton crop-productivity compared to the average", subtitle = "By quadrant", x = "Crop-productivity pr. worker", y = "Crop-productivity pr. acre", color = "Quadrant")

ggsave("plots/Mean_cotton_crop-zscores.pdf", height= 6, width = 10)
...
### Value weighted
Now we can weigh the scores by market value of the crops to create a aggregated productivity score pr. worker and pr. acre. Furthermore a calculation of the value of the harvest (by crop and in total) will also be made.
```{r}
# Weighted scores by value
# Sources:
# Sugar - Willerslev brown sugar pd. price (1792), p. 63
# Rum - Vore gamle tropekolonier price pr. barrel (1793-1796), p. 228
# Cotton - Vore gamle tropekolonier price pr. sack (1793-1796), p. 228 (1 sack = ca. 210 pd.)

# First we calculate the total harvest value - by crop and in total
productivity <- productivity %>%
  mutate(avg_harvest_sugar_value = (avg_harvest_sugar*1000) * 11.5, # First we convert to pd. 
         avg_harvest_rum_value = avg_harvest_rum * (14400*0.75), # 0.75 to covert Rd.v.c. to Rdl.
         avg_harvest_cotton_value = (avg_harvest_cotton/218)*(9600*0.75)) %>% # 0.75 to covert Rd.v.c. to Rdl.
  rowwise() %>% 
  mutate(avg_harvest_value = sum(avg_harvest_sugar_value, avg_harvest_rum_value, avg_harvest_cotton_value, na.rm = T))

# Then we calculate value by worker and acre - by crop and aggregated.
productivity <- productivity %>% 
  mutate(worker_productivity_sugar_value = worker_productivity_sugar * 11.5,
         worker_productivity_rum_value = worker_productivity_rum * (14400*0.75),
         worker_productivity_cotton_value = (worker_productivity_cotton/218) * (9600*0.75),
         acre_productivity_sugar_value = acre_productivity_sugar * 11.5,
         acre_productivity_rum_value = acre_productivity_rum * (14400*0.75),
         acre_productivity_cotton_value = (acre_productivity_cotton/218)* (9600*0.75)) %>% 
  rowwise() %>% 
  mutate(agr_productivity_worker = sum( worker_productivity_sugar_value,worker_productivity_rum_value,worker_productivity_cotton_value, na.rm = T), # Aggregate scores
         agr_productivity_acre = sum(acre_productivity_sugar_value,acre_productivity_rum_value,acre_productivity_cotton_value, na.rm = T)) %>% 
  ungroup() %>% 
  mutate(across(c(avg_harvest_sugar_value:agr_productivity_acre), ~ .x / 96)) # Covert skilling to rds
```

```{r}
# A quick calculation of mean value-productivity scores
paste("Mean value-productivity scores:")
paste("Pr. worker - Sugar:", mean(productivity$worker_productivity_sugar_value, na.rm = T), "Pd. pr. worker", sep=" ")
paste("Pr. worker - Rum:", mean(productivity$worker_productivity_rum_value, na.rm = T), "XXX pr. worker", sep=" ")
paste("Pr. worker - Cotton:", mean(productivity$worker_productivity_cotton_value, na.rm = T), "Pd. pr. worker", sep=" ")
paste("Pr. acre - Sugar:", mean(productivity$acre_productivity_sugar_value, na.rm = T), "Pd. pr. acre", sep=" ")
paste("Pr. acre - Rum:",mean(productivity$acre_productivity_rum_value, na.rm = T), "XXX pr. acre", sep=" ")
paste("Pr. acre - Cotton:", mean(productivity$acre_productivity_cotton_value, na.rm = T), "Pd. pr. acre", sep=" ")

paste("")

#And median scores
paste("Median value-productivity scores:")
paste("Pr. worker - Sugar:", median(productivity$worker_productivity_sugar_value, na.rm = T), "Pd. pr. worker", sep=" ")
paste("Pr. worker - Rum:", median(productivity$worker_productivity_rum_value, na.rm = T), "XXX pr. worker", sep=" ")
paste("Pr. worker - Cotton:", median(productivity$worker_productivity_cotton_value, na.rm = T), "Pd. pr. worker", sep=" ")
paste("Pr. acre - Sugar:", median(productivity$acre_productivity_sugar_value, na.rm = T), "Pd. pr. acre", sep=" ")
paste("Pr. acre - Rum:", median(productivity$acre_productivity_rum_value, na.rm = T), "XXX pr. acre", sep=" ")
paste("Pr. acre - Cotton:", median(productivity$acre_productivity_cotton_value, na.rm = T), "Pd. pr. acre", sep=" ")
```

The value-productivity also makes us able to calculate a aggregated worker and acre score
```{r}
paste("Aggregated value-productivity scores:")
paste("Pr. worker - Mean:", mean(productivity$agr_productivity_worker, na.rm = T), "Rdl. pr. worker", sep=" ")
paste("Pr. worker - Median:", median(productivity$agr_productivity_worker, na.rm = T), "Rdl. pr. worker", sep=" ")
paste("Pr. acre - Mean:", mean(productivity$agr_productivity_acre, na.rm = T), "Rdl. pr. acre", sep=" ")
paste("Pr. acre - Median:", median(productivity$agr_productivity_acre, na.rm = T), "Rdl. pr. acre", sep=" ")
```

We can also quickly calculate the total harvest value of St. Croix
```{r}
paste("1792 harvest value:")
paste("")
paste(sum(productivity$avg_harvest_sugar, na.rm = T)*1000,"Pds. of sugar priced at 11.5 skilling pr. unit:", round(sum(productivity$avg_harvest_sugar_value, na.rm = T), 2), "Rdl.", sep=" ")
paste(sum(productivity$avg_harvest_rum, na.rm = T),"Barrels of rum priced at", round(14400*0.75,2),"skilling (150 Rdl.) pr. unit:", round(sum(productivity$avg_harvest_rum_value, na.rm = T), 2), "Rdl.", sep=" ")
paste(sum(productivity$avg_harvest_cotton, na.rm = T),"Pds. of cotton priced at", round(44*0.75,2) ,"skilling pr. unit:", round(sum(productivity$avg_harvest_cotton_value, na.rm = T), 2), "Rdl.", sep=" ")
paste("")
paste("Total harvest value of St. Croix in 1792:", round(sum(productivity$avg_harvest_value, na.rm = T), 2), "- ca." ,round(sum(productivity$avg_harvest_value, na.rm = T)/1000000, 2), "Million Rdl.", sep=" ")
```

This is not to far form the number quoted in Vore Tropekolonier for the years 1793-1796 (3.542 mil Rdl.). Oxholm also put the value at ca. 4 mil Rdl. for 1793. Although 1793 was a extraordinary good year according to himself.

Let us also calculate the aggregated productivity scores and the median / harvest values but for the different plantation types.
```{r}
productivity %>%
  mutate_at(vars(acre_sugar, acre_cotton), ~replace_na(., 0)) %>% 
  mutate(plantation_type = ifelse(acre_sugar > 0 & acre_cotton <= 10 , "Sugar", ifelse(acre_cotton > 0 & acre_sugar <= 10, "Cotton", "Mixed"))) %>%
  group_by(plantation_type) %>% 
  summarise(count = n(),
            mean_harvest_val = mean(avg_harvest_value),
            median_harvest_val = median(avg_harvest_value),
            mean_agr_productivity_acre = mean(agr_productivity_acre),
            median_agr_productivity_acre = median(agr_productivity_acre),
            mean_agr_productivity_worker = mean(agr_productivity_worker),
            median_agr_productivity_worker = median(agr_productivity_worker))
```

### Plantation profitability
We now know the average harvest value of the different plantations, but what of their profitability? Were sugar plantations pure profit, or was the expenses so high that it took most of it?

Using Sveistrup's calculations based on Oxholm and West, we have should be able to calculate the expenses of each plantation.
First West estimated the expens pr. slave to be ca. 38 Rdl. V.C. pr. year. This should be easy to calculate.
```{r}
productivity <- productivity %>%
  mutate(expenses = total*(38*0.75)) # expenses pr. enslaved
```

Next Sveistrup we can use the 1792 census data to calculate head and property tax. First we recalculate the taxes to whole Rdl. from Rdl and skilling V.C. 
```{r}
temp <- quadrant_data %>% 
  filter(year == 1792,
         !(quadrant == "Christiansted" | quadrant == "Frederiksted")) %>%
  mutate(tax_enslaved_skilling = replace_na(tax_enslaved_skilling, 0),
         tax_land_skilling = replace_na(tax_land_skilling, 0)) %>%
  mutate(head_tax = (tax_enslaved_rigsdaler+(tax_enslaved_skilling/96))*0.75,
         land_tax = (tax_land_rigsdaler+(tax_land_skilling/96))*0.75) %>% 
  mutate(enslaved_influx_price = (enslaved_bosal*400)*0.75)
```

Now we can transfer these results over to our total expenses.
```{r}
temp <- temp %>% # only selecting relevant columns
  select(quadrant, estimated_size, head_tax, land_tax, enslaved_influx_price)

productivity_temp <- productivity %>% # joining with productivity data
  left_join(temp, by = "quadrant")

# Now we calculate the expenses proportional to each element
productivity_temp <- productivity_temp %>%
  mutate(proportional_head_tax = (total / sum(total[quadrant == quadrant])) * head_tax,
         proportional_land_tax = (acre_total / sum(acre_total[quadrant == quadrant])) * land_tax,
         proportional_enslaved_price = (total / sum(total[quadrant == quadrant])) * enslaved_influx_price) %>% 
  mutate(wage_expenses = (total / sum(total)) * 100000,
         inventory_expenses = (total / sum(total)) * 100000)
  
  # Step 3: Add these values to the "expenses" column
productivity_temp <- productivity_temp %>% 
  mutate(expenses = expenses + proportional_head_tax + proportional_land_tax + proportional_enslaved_price + wage_expenses + inventory_expenses,
         profit = avg_harvest_value - expenses) 
```

```{r}
print(paste("The median harvest value for plantations was ca.:", round(median(productivity_temp$avg_harvest_value))))
print(paste("The median expenses for plantations was ca.:", round(median(productivity_temp$expenses))))

print(paste("The median profit for plantations was ca.:", round(median(productivity_temp$profit))))
...
productivity_temp %>%
  ...
  summarise(count = n(),
            ...
            median_expenses = median(expenses),
            mean_profit = mean(profit),
            median_profit = median(profit))
```


## Plotting productivity results
Now we have our productivity scores, we can now plot our results. We will do this both "normally" and spatially.

## Normally
The following will plot the productivity scores from above.
```{r total_harvest_value}
productivity %>% 
  ggplot() +
  geom_histogram(aes(x=avg_harvest_value, fill=quadrant), binwidth = 5000) +
  labs(title="Average harvest value - Histogram", subtitle = "By quadrant", fill="Quadrant", x="Total harvest value (Rdl.)")
```

```{r harvest_value_crop}
productivity %>% 
  ggplot() +
  geom_histogram(aes(x=avg_harvest_sugar_value, fill=quadrant), binwidth = 2500) +
  labs(title="Average sugar harvest value - Histogram", subtitle = "Binsize = 2500", fill="Quadrant", x="Total sugar harvest value (Rdl.)")

productivity %>% 
  ggplot() +
  geom_histogram(aes(x=avg_harvest_rum_value, fill=quadrant), binwidth = 1250) +
  labs(title="Average rum harvest value - Histogram", subtitle = "Binsize = 1250", fill="Quadrant", x="Total rum harvest value (Rdl.)")

productivity %>% 
  ggplot() +
  geom_histogram(aes(x=avg_harvest_cotton_value, fill=quadrant), binwidth = 250) +
  labs(title="Average cotton harvest value - Histogram", subtitle = "Binsize = 250", fill="Quadrant", x="Total cotton harvest value (Rdl.)")
```

```{r harvest_value_plt_type}
# Mixed plantations set to have more than 10 acres s for both sugar and cotton
productivity %>%
  mutate_at(vars(acre_sugar, acre_cotton), ~replace_na(., 0)) %>% 
  mutate(plantation_type = ifelse(acre_sugar > 0 & acre_cotton <= 10 , "Sugar", ifelse(acre_cotton > 0 & acre_sugar <= 10, "Cotton", "Mixed"))) %>% 
  ggplot() +
  geom_histogram(aes(x=avg_harvest_value, fill=plantation_type), binwidth = 5000) +
  labs(title="Average harvest value - Histogram", subtitle = "By plantation type", fill="Plantation Type", x="Total harvest value (Rdl.)")
```


```{r productivity__harvest_values}
productivity %>% 
  ggplot() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  geom_point(aes(x=agr_productivity_worker, y = agr_productivity_acre, color = quadrant)) +
  labs(title="Value created pr. acre and enslaved, 1792", subtitle = "Based on value of average harvest", color = "Quadrant", x="Average value created pr. enslaved (Rdl.)", y = "Average value created pr. acre (Rdl.)") +
  scale_y_continuous(breaks = seq(0, 1050, by=100), limits = c(0,1050)) +
  scale_x_continuous(breaks = seq(0, 1050,by=100), limits = c(0,1050))
```

```{r productivity_value_to_average}
mean_worker_value <- mean(productivity$agr_productivity_worker, na.rm = T)
sd_worker_value <- sd(productivity$agr_productivity_worker, na.rm = T)

mean_acre_value <- mean(productivity$agr_productivity_acre, na.rm = T)
sd_acre_value <- sd(productivity$agr_productivity_acre, na.rm = T)

productivity$Z_worker_value <- z_score(productivity$agr_productivity_worker, mean_worker_value, sd_worker_value)
productivity$Z_acre_value <- z_score(productivity$agr_productivity_acre, mean_acre_value, sd_acre_value)

productivity %>%
  ggplot() +
  geom_point(aes(x=Z_worker_value, y=Z_acre_value, color=quadrant)) +
  geom_hline(aes(yintercept=0)) +
  geom_vline(aes(xintercept=0)) +
  labs(title = "Value-productivity compared to average", x = "Value-productivity created pr. worker", y = "Value-productivity created pr. acre", color = "Quadrant")

ggsave("plots/value-zscores.pdf", height= 6, width = 10)
```

```{r mean_median_productivity_value_to_average}
productivity %>%
  group_by(quadrant) %>%
  summarize(
    mean_worker = mean(Z_worker_value, na.rm = TRUE),
    mean_acre = mean(Z_acre_value, na.rm = TRUE),
    median_worker = median(Z_worker_value, na.rm = TRUE),
    median_acre = median(Z_acre_value, na.rm = TRUE)) %>%
  pivot_longer(cols = c("mean_worker", "mean_acre", "median_worker", "median_acre"),  # Selects columns starting with "age_"
                          names_to = c("mean_median", "worker_acre"),  # Separates the column names into gender and age group
                          names_pattern = "(mean|median)_(worker|acre)",  # Regex pattern to capture gender and age group
                          values_to = "count") %>% 
  ...
  geom_hline(aes(yintercept=0)) +
  geom_vline(aes(xintercept=0)) +
  labs(title = "Mean and median value-productivity compared to the average", subtitle = "By quadrant", x = "Value productivity pr. worker", y = "Value productivity pr. acre", color = "Quadrant", shape = "Mean or median")

ggsave("plots/Mean_value-zscores.pdf", height= 6, width = 10)
```

## Spatially
First we need to combine our productivity data with our spatial data.
```{r}
plantations_1792_union_temp <- plantations_1792 %>% 
  group_by(plantation, owner, quadrant) %>% 
  summarise(geometry = st_union(geometry)) %>%
...
productivity_plantation <- plantations_1792_union_temp %>%
  select("plantation", "owner", "quadrant") %>% 
  inner_join(productivity, by = c("plantation", "owner", "quadrant"))
  
productivity_quadrant <- productivity %>%
  summarise() %>% 
  left_join(quadrants_cadastre_oxholm_1799) %>% 
  st_as_sf()
```

```{r avg_score_plantations}
ggplot(productivity_plantation)+
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=avg_harvest_value)) +
  scale_fill_viridis() +
  labs(title = "Expected harvest value on St. Croix, 1790's", 
       subtitle = "Based on the expected average harvest from the 1792 census",
       fill = "Total harvest value (Rdl.)")

ggsave("maps/expected_harvest_value_plantation.pdf", height= 6, width = 10)

productivity_plantation %>% 
  filter(agr_productivity_worker < 600) %>% 
ggplot()+
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=agr_productivity_worker)) +
  scale_fill_viridis() +
  labs(title = "Value created pr. enslaved on St. Croix, 1790's", 
       subtitle = "Based on the expected average harvest from the 1792 census",
       fill = "Value pr. enslaved (Rdl.)")

ggsave("maps/value_enslaved_plantation.pdf", height= 6, width = 10)

productivity_plantation %>% 
  filter(agr_productivity_acre < 400) %>% 
ggplot()+
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=agr_productivity_acre)) +
  scale_fill_viridis() +
  labs(title = "Value created pr. acre on St. Croix, 1790's", 
       subtitle = "Based on the expected average harvest from the 1792 census",
       fill = "Value pr. acre (Rdl.)")

ggsave("maps/value_acre_plantation.pdf", height= 6, width = 10)
```

```{r work_prod_plantation}
productivity_plantation %>% 
  filter(worker_productivity_sugar < 5000) %>% 
  ggplot() +
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=worker_productivity_sugar)) +
  scale_fill_viridis()

productivity_plantation %>%
  filter(worker_productivity_rum < 2.5) %>% 
  ggplot()+
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=worker_productivity_rum)) +
  scale_fill_viridis()

productivity_plantation %>% 
  filter(worker_productivity_cotton > 0) %>% 
  ggplot()+
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=worker_productivity_cotton)) +
  scale_fill_viridis()
```

```{r acre_prod_plantation}
productivity_plantation %>% 
  filter(acre_productivity_sugar < 2500) %>% 
  ggplot() +
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=acre_productivity_sugar)) +
  scale_fill_viridis()

productivity_plantation %>%
  filter(acre_productivity_rum < .95) %>% 
  ggplot()+
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=acre_productivity_rum)) +
  scale_fill_viridis()

productivity_plantation %>% 
  filter(acre_productivity_cotton > 0) %>% 
  ggplot()+
  geom_sf(data=St_Croix)+
  geom_sf(aes(fill=acre_productivity_cotton)) +
  scale_fill_viridis()
```

## What makes a plantation productive
### Ownership (family) or nationality
When calculating what makes a plantation productive, it might also be a good idea to first look at plantations under different family or national ownership. Were there patterns in some better that others at being productive?

Let us start with nationality and the aggregated scores (with unknown being filtered out).
```{r}
productivity %>% 
  filter(!is.na(owner_nationality)) %>% 
  group_by(owner_nationality) %>%
  summarise(n = n(),
            worker_prod = median(agr_productivity_worker),
            acre_prod = median(agr_productivity_acre),
            sugar_acre = mean(acre_sugar, na.rm = T),
            cotton_acre = mean(acre_cotton, na.rm = T))
```

Now let us look at the indvidual crop-productivity scores
```{r}
# Sugar and rum crop-productivity
productivity %>% 
  filter(!is.na(owner_nationality)) %>% 
  group_by(owner_nationality) %>%
  summarise(n = n(),
            sugar_acre = mean(acre_sugar, na.rm = T),
            sugar_worker_prod = median(worker_productivity_sugar, na.rm = T),
            sugar_acre_prod = median(acre_productivity_sugar, na.rm = T),
            rum_worker_prod = median(worker_productivity_rum, na.rm = T),
            rum_acre_prod = median(acre_productivity_rum, na.rm = T))
```
...
productivity %>% 
  ...
  summarise(n = n(),
            cotton_acre = mean(acre_cotton, na.rm = T),
            cotton_worker_prod = median(worker_productivity_cotton, na.rm = T),
            cotton_acre_prod = median(acre_productivity_cotton, na.rm = T))
```

Let us also take a look at the most productive families.
```{r}
productivity %>% 
  filter(!is.na(owner_surname)) %>% 
  group_by(owner_surname) %>%
  summarise(n = n(),
            worker_prod = median(agr_productivity_worker),
            acre_prod = median(agr_productivity_acre),
            sugar_acre = sum(acre_sugar, na.rm = T),
            cotton_acre = sum(acre_cotton, na.rm = T)) %>% 
  top_n(12, worker_prod) %>%  # CHANGE THIS TO VIEW TOP 10 FAMILIES FOR THE DIFFERENT CATEGORIES.
  arrange(desc(worker_prod))
```

```{r}
# Sugar/rum crop-productivity
productivity %>% 
  filter(!is.na(owner_surname)) %>% 
  group_by(owner_surname) %>%
  summarise(n = n(),
            ...
            rum_worker_prod = median(worker_productivity_rum, na.rm = T),
            rum_acre_prod = median(acre_productivity_rum, na.rm = T)) %>% 
  top_n(12, sugar_acre_prod) %>%  # CHANGE THIS TO VIEW TOP 10 FAMILIES FOR THE DIFFERENT CATEGORIES.
  arrange(desc(sugar_acre_prod))
...
productivity %>% 
  ...
  summarise(n = n(),
            ...
            cotton_worker_prod = median(worker_productivity_cotton, na.rm = T),
            cotton_acre_prod = median(acre_productivity_cotton, na.rm = T)) %>% 
  top_n(12, cotton_worker_prod) %>%  # CHANGE THIS TO VIEW TOP 10 FAMILIES FOR THE DIFFERENT CATEGORIES.
  arrange(desc(cotton_worker_prod))
```

### Correlation matrix
With that analysed, we can now turn to creating a correlation matrix. To do this we need to select only the relevant data we need from the productivity data frame and then combine it with some other data that we still need to format.

This data includes:
* Total amount of enslaved / total field workers
* Total plantation size
* Total wind- and animal mills on each plantation (according to Oxholm's map)
* Soil quality (ph + salinity)
  + We probably need to take either the mean or most dominating values as the data is not formatted by plantation (obviously)
  + We also till need to download and format this in QGIS before importing it to R.
* Avg. temperature
* Avg. rainfall
* Aeration
* Drainage
...
**Selecting relevant dataframe columns**
The first step we need to do is count how many wind- and animals mills there are pr. plantation. As was shown at the very start of this rmarkdown, we've already extracted the mill locations from Oxholm's 1799 map. Now we need just to combine the data with our 1792 plantation shapefile.
```{r}
plantations_1792_union$horsemills <- st_intersects(plantations_1792_union, horsemills_oxholm_1799) |> lengths()
plantations_1792_union$windmills <- st_intersects(plantations_1792_union, windmills_oxholm_1799) |> lengths()

# selecting only the relevant data
temp <- plantations_1792_union %>%
  select(plantation, owner, quadrant, acre_total, acre_for_enslaved, horsemills, windmills)

correlation_productivity <- temp %>% 
  left_join(productivity)
```

Now let us select the relevant data from the productivity matrix...
```{r}
# Selecting relevant data
correlation_productivity <- correlation_productivity %>% 
  select(plantation, # will be delected later. Only for joining purposes
         owner, # will be delected later. Only for joining purposes
         quadrant, # will be delected later. Only for joining purposes
         field_workers,
         total,
         acre_sugar,
         acre_cotton,
         ...
         horsemills,
         windmills,
         agr_productivity_worker,
         agr_productivity_acre)
```


**Downloading and combining the USDA soil data**
```{r, eval=F}
USDA_rain <- st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_annual_p_1.gpkg") %>%
  rename("avg_rain" = "X_mean") %>% 
  rowid_to_column("ID")
USDA_temp <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_annual_t_2.gpkg")) %>% 
  select(-geom) %>%
  rename("avg_temp" = "X_mean") %>%
  rowid_to_column("ID")
USDA_bulk_density <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_bulk_densi.gpkg")) %>% 
  select(-geom) %>%
  rename("bulk_density" = "X_mean") %>%
  rowid_to_column("ID")
USDA_calcium <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_calcium_pc.gpkg")) %>% 
  select(-geom) %>%
  rename("calcium" = "X_mean") %>%
  rowid_to_column("ID")
USDA_cation <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_cation_ex_.gpkg")) %>% 
  select(-geom) %>%
  rename("cation" = "X_mean") %>%
  rowid_to_column("ID")
USDA_clay_pct <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_clay_pct.gpkg")) %>% 
  select(-geom) %>%
  rename("clay_pct" = "X_mean") %>%
  rowid_to_column("ID")
USDA_groundwater <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_groundwate.gpkg")) %>% 
  select(-geom) %>%
  rename("groudwater_depth" = "X_mean") %>%
  rowid_to_column("ID")
USDA_ph <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_ph.gpkg")) %>% 
  select(-geom) %>%
  rename("ph" = "X_mean") %>% 
  rowid_to_column("ID")
USDA_restrictive <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_restrictiv.gpkg")) %>% 
  select(-geom) %>%
  rename("restrictive_soil_depth" = "X_mean") %>%
  rowid_to_column("ID")
USDA_salination <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_salination.gpkg")) %>% 
  select(-geom) %>%
  rename("salination" = "X_mean") %>%
  rowid_to_column("ID")
USDA_sand_pct <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_sand_pct.gpkg")) %>% 
  select(-geom) %>%
  rename("sand_pct" = "X_mean") %>%
  rowid_to_column("ID")
USDA_silt_pct <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_silt_pct.gpkg")) %>% 
  select(-geom) %>%
  rename("silt_pct" = "X_mean") %>%
  rowid_to_column("ID")
USDA_wind_erosion <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_wind_erodi.gpkg")) %>% 
  select(-geom) %>%
  rename("wind_erosion" = "X_mean") %>%
  rowid_to_column("ID")
USDA_water_content <- as.data.frame(st_read("../QGIS/Data/USDA_weighted_avg/plantation_1792_USDA_soil_water_cont.gpkg")) %>% 
  select(-geom) %>%
  rename("water_content" = "X_mean") %>%
  rowid_to_column("ID")

plantation_elevation <- as.data.frame(st_read("../QGIS/Data/Plantations_1792_elevation.gpkg")) %>%
  select(-geom) %>% 
  select(-c("X_mean", "X_min", "X_max")) %>%
  rename("elevation_median" = "X_median")
```

Now we can combine it into one large shapefile
```{r}
USDA_soil <- USDA_rain %>% 
  left_join(USDA_temp) %>% 
  left_join(USDA_bulk_density) %>% 
  left_join(USDA_calcium) %>% 
  left_join(USDA_cation) %>% 
  left_join(USDA_clay_pct) %>% 
  left_join(USDA_groundwater) %>% 
  left_join(USDA_ph) %>% 
  left_join(USDA_restrictive) %>% 
  left_join(USDA_salination) %>% 
  left_join(USDA_sand_pct) %>% 
  left_join(USDA_silt_pct) %>% 
  left_join(USDA_wind_erosion) %>% 
  left_join(USDA_water_content)

USDA_soil <- USDA_soil %>% 
  left_join(plantation_elevation) %>% 
  select(-ID)
```

Now we can combine the data with out productivity correlation_productivity data_frame from earlier.
```{r}
temp <- as.data.frame(USDA_soil) %>% 
  select(-geom)
...
**Correlation matrix**
With all that done, we can now create our correlation matrix and calculate what has the biggest impact on productivity.

First we remove the non-numeric columns from the correlation_productivity data frame

```{r}
correlation_productivity_matrix <- correlation_productivity %>%
  as.data.frame() %>% 
  select(-quadrant,
         -owner,
         -plantation,
...
We also need to change all NA's to 0. This should no impact any of our soil data and artificially push our results in incorrect directions as there are no NA's there.
```{r}
correlation_productivity_matrix <- correlation_productivity_matrix %>%
  mutate(across(everything(), ~ replace_na(., 0))) # setting NA's to 0
```

Now we can do the actual correclation calculations
```{r}
correlation_productivity_matrix <- cor(correlation_productivity_matrix)
```

We're only interested in the columns on productivity, so let us sort all other columms out.



Now we can plot it...
```{r}
corrplot(correlation_productivity_matrix, method="color", tl.cex = .75)
```

So according to soil is actually not the main decider in relation to plantation productivity. There are some aspects that show a small correlation, but for the most part it is irrelevant. There does seem to be some connection to larger (in terms of total enslaved not acres) plantations having higher productivity, showcasing that economy of scale probably had some impact.

### What can't we account for?
The correlation matrix and other reasons for heightened or lowered productivity studied above, are not the only factors that might influence how productive plantations are. The following includes some potential factors that were not examined, either due to lack of time, data or reliable detailed sources.

* Different crop types
  + Oxholm mentions a new type of sugar cane
* Plantation equipment (beside wind- and animal mills)
* Accurate yearly weather accounts
* Soil depletion or changes since 1792 to current USDA information.
* Soil values were in large part determined based on Food and Agricultural Organisation of the United Nations. There might be other soil qualities influencing the crops.
* Soil aeration is not something that USDA has data on (weird?!), so this might have an impact. Seeing as bulk density shows a minor correlation and is one half of the equation to calculating porosity, there might be something to this.

It might also be relevant to ask the question: **is more productivity is always better?** On the islands there was a self destructive tendency to value short term profit over long term sustainability. Were plantations with high productivity a indicator of this trend? 

# Plantation size
Both the 1792 and 1804 census can be used to examine the general size of plantations.
```{r}
census_1804 %>%
  ggplot(aes(x = field_total)) +
  geom_histogram(binwidth = 50) +
  scale_x_continuous(breaks =  seq(0,750,50)) +
  labs(title="Histogram of plantation sizes, 1804", x = "Total size (acres)")

ggsave("plots/plantation_size_1804_hist.pdf", height= 5, width = 8)

census_1792 %>%
  ggplot(aes(x = acre_total)) +
  geom_histogram(binwidth = 50) +
  scale_x_continuous(breaks =  seq(0,1100,100)) +
  labs(title="Histogram of plantation sizes, 1792", x = "Total size (acres)")

ggsave("plots/plantation_size_1792_hist.pdf", height= 5, width = 8)
```
